...
import { ArrowUp, Check, Edit, FolderOpen, PanelRight, Plus, Refresh, TelaCheia, Trash, X } from "./icons";
import { campoPrompt, larguraNumero, numeroPilula } from "./Composer";
import { btn, btnPrimary, SAMPLERS } from "./ImagensUi";
...
export default function ImagensView(props: {
  ...
  const carregarLocal = useCallback(async () => {
    try {
      const novo = await api.get<EstadoImagens>("/imagens/estado");
      setSt(novo);
  ...
  async function gerar(confirm = false) {
    ...
    try {
      ...
      await api.post(`/imagens/${conv}/gerar`, {
        prompt,
        // offload/flash attention são do modelo (Motor e modelos): o global não passa por cima
  ...
  async function anexar(files: FileList | null) {
    ...
    try {
      for (const f of Array.from(files ?? [])) {
        // No navegador não há caminho do arquivo: vai uma cópia para <pasta de imagens>/referencias.
        const path = await uploadReferencia(f);
        if (velha) {
  ...
  return (
    <>
      ...
      <BarraTopo
        contagem={lotes.reduce((n, l) => n + (l.resposta.meta as LoteMeta).images.length, 0)}
      ...
      <div className="shrink-0 px-5 pb-4">
        <div className="mx-auto max-w-[1400px]">
          ...
          {abrirMotor && (
            <ImagensMotor st={st} onMudou={carregarLocal} onError={mostrarErro} onFechar={() => setAbrirMotor(false)} />
          )}
            ...
            <div className="mt-2.5 flex flex-wrap items-center gap-2 border-t border-line pt-2.5 text-xs">
              ...
              <div className="ml-auto flex min-w-0 items-center gap-3">
                ...
                <button
                  ...
                  title={!models.length ? "Escolha um modelo em Parâmetros" : st.image_busy || ocupado ? "Já tem imagem sendo gerada" : "Gerar (Enter)"}
...
function Ajustes(props: {
  ...
  async function salvarPrazo(dias: number) {
    ...
    try {
      await api.put("/imagens/ajustes", { ...o, descarte_dias: dias });
    } catch (e: any) {
      props.onError(e.message);
    }
  }
  ...
  return (
    <aside className="flex w-[292px] shrink-0 flex-col gap-[18px] overflow-y-auto border-l border-line bg-side p-4 text-xs">
      <div className="flex items-center gap-2">
        ...
        <button onClick={props.onMotor} className="ml-auto text-[11.5px] text-faint hover:text-fg">Motor e modelos</button>
        <button onClick={props.onFechar} title="Esconder os parâmetros" className="rounded-[7px] p-1 text-faint hover:bg-raised hover:text-fg">
          <X className="size-3.5" />
        </button>
      </div>
...
function Liquido({ fracao, sPasso, restante, fase }: { fracao: number; sPasso?: number; restante?: number; fase?: string }) {
...
const listras = "bg-[repeating-linear-gradient(135deg,var(--color-raised)_0_7px,var(--color-surface)_7px_14px)]";
    ...
    >
      ...
      {comPrevia && (
        ...
        <svg
          ...
          aria-valuemin={0}
          aria-valuemax={100}
          className="absolute right-[5px] top-[5px] size-[30px]"
        >
          ...
          <circle
            ...
            className="stroke-accent transition-[stroke-dasharray] duration-1000 ease-out"
          />
      ...
      <figcaption className={`absolute inset-x-0 bottom-0 flex items-end gap-1.5 px-2 pt-6 pb-1.5 font-mono text-[10.5px] ${
        ...
        >
          <span className="truncate">{img.model_name || "—"}</span>
