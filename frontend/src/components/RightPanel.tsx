...
import { Activity, Clipboard, Expandir, GitBranch, Globe, Info, Recolher, Terminal, X } from "./icons";
...
export const WIDTH: Record<RightTab, number> = { info: 288, browser: 520, servers: 288, plans: 440, changes: 520, terminal: 560 };
...
/** O que cada aba sinaliza: ponto verde (navegador aberto) ou contador. */
    ...
    <div className="ml-auto flex shrink-0 items-center gap-0.5" role="toolbar" aria-label="Painéis">
      {[...TABS, ...(props.extras ?? [])].map((t) => {
        ...
        return (
          ...
          >
            {t.icon}
            {b && <span className={`absolute grid place-items-center ${t.id === "browser" ? "top-0 right-0" : "-top-[3px] -right-[3px]"}`}>{b}</span>}
          </button>
        );
      })}
    </div>
