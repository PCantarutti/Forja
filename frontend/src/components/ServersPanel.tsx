...
export default function ServersPanel(props: {
  ...
  async function refresh() {
    try {
      const [r, d] = await Promise.all([
        api.get<{ servers: ServerInfo[]; runner: string }>("/servers"),
  ...
  function cartaoProcesso(s: ServerInfo, concluido = false) {
    const onde = s.where === "host" ? "seu sistema" : "container";
    ...
    return (
      <Cartao
        ...
        titulo={s.name}
        sub={`Processo · ${onde} · ${estado}${s.pid ? ` · pid ${s.pid}` : ""}`}
  ...
  return (
    <div className="flex h-full flex-col gap-2 overflow-y-auto p-3 text-xs">
      ...
      <div className="mt-1 text-faint">Subagentes e processos de fundo. Runner: {runner || "…"}.</div>
    </div>
  );
}
