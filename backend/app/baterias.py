...
from . import config, db, llm, shell
...
def testar_codigo(codigo: str, dica: str, chave: str, conv: int | str | None = None, bateria: str = "") -> dict:
    ...
    if not (vivo and nome in _PORTAS):
        ...
        exe = f'"{sys.executable}"'  # o http.server roda no container, com o Python do backend
        ...
        try:
            shell._local_start(nome, f"{exe} -m http.server {_PORTAS[nome]} --bind 127.0.0.1", pasta)
        finally:
