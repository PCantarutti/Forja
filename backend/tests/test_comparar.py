...
SEM_LOCAL = pytest.mark.skipif(importlib.util.find_spec("app.localai") is None,
                               reason="IA local (.gguf) só existe no Forja desktop")
...
def test_testar_codigo_html_sobe_servidor_e_python_vira_comando(tmp_path, monkeypatch):
    ...
    subidos = []
    monkeypatch.setattr(shell, "_local_start", lambda nome, cmd, cwd: subidos.append((nome, cmd, cwd, shell.CONV.get())))
    ...
    assert subidos[0][1].startswith('"')  # roda no bash do container, sem o & do PowerShell
