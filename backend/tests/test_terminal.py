...
O mesmo bug existia no `tools/forja_runner.py`, que é o caminho usado quando o runner está ligado
(ver tools/test_forja_runner.py). Correção correspondente no repo Desktop: 4b81b35.
"""
import codecs
import threading
import time
...
@pytest.fixture
def term():
    t = terminal.LocalTerm.__new__(terminal.LocalTerm)  # sem subir bash de verdade
    t.proc = FakeProc()
    t.buf = ""
    t.written = 0
...
def test_terminal_sobrevive_ao_estouro_do_buffer(term):
    ...
    assert term.poll(cursor)["text"] == "depois-do-estouro"
