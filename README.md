...
- **Hooks**: `.forja/hooks.json` na pasta da conversa roda comandos depois de uma ferramenta, ex.: `{"post_tool": [{"tools": ["write_file", "edit_file"], "command": "npx prettier --write \"{path}\""}]}`. A saída é anexada ao resultado para o modelo ver. **A pasta precisa estar em Configurações › Permissões › Pastas confiáveis** (subpastas herdam): o arquivo vem junto num `git clone`, então um repositório de terceiros não roda comando nenhum até você liberar. Enquanto não liberar, a conversa avisa e os hooks ficam parados.
...
O PDF sai pelo mesmo Chromium que o backend já usa para o navegador integrado, pelo caminho de um *imprimir para PDF*: sem biblioteca de PDF no meio, com a tipografia e as tabelas de um navegador de verdade.
